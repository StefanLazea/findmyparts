...
const savePart = async (req, res) => {
  ...
  if (_.isNil(part)) {
    return res
      .status(400)
      .send({ message: "Please send a body or a correct format" });
  }
...
const deletePart = async (req, res) => {
  ...
  await part.destroy().then(() => {
    return res.send({ message: "Part deleted" });
  });
};

const getOnePartDetails = async (req, res) => {
  if (_.isNil(req.params.partId)) {
    return res.status(400).send({ message: "Please provide part id." });
  }
